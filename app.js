...
const C = (key) => key.split('.').reduce((o, k) => o && o[k], CONTENT);
...
function TeamAvatar({ photo, initials }) {
  ...
  useEffect(() => {
    ...
    function attempt() {
      if (i >= PHOTO_EXTS.length || cancelled) return;
      const url = './assets/Team/' + photo + '.' + PHOTO_EXTS[i++];
      const img = new Image();
      img.onload = () => { if (!cancelled && img.naturalWidth > 0) setImgSrc(url); };
      img.onerror = attempt;
      img.src = url;
    }
    attempt();
...
function Reveal({ children, className = '', delay = 0 }) {
  ...
  const cls = 'reveal' + (delay ? ' reveal-delay-' + delay : '') + (className ? ' ' + className : '');
...
function SocialLinks({ items, className }) {
  return html`<div class=${'footer-social ' + (className || '')}>
    ${(items || shared.socials).map(s => html`
      <a key=${s.label} href=${s.url} target="_blank" class="social-link" aria-label=${s.label}
         dangerouslySetInnerHTML=${{ __html: SOCIAL_SVG[s.icon] || '' }} />`)}
  </div>`;
}
...
function Navbar({ current }) {
  ...
  const nav = (to, label) => html`<a href=${'#/' + to} class=${current === to ? 'active' : ''} onClick=${() => setOpen(false)}>${label}</a>`;
  return html`<header class=${'navbar' + (scrolled ? ' scrolled' : '')}>
    <a href="#/" class="logo-link"><${SmartImg} src="./assets/Logo.png" alt="Cenoir Studios" className="logo" /><span class="brand">Cenoir Studios</span></a>
    <nav class=${'nav-links' + (open ? ' open' : '')}>${nav('', 'Home')}${nav('about', 'About')}${nav('projects', 'Projects')}${nav('news', 'News')}${nav('careers', 'Careers')}</nav>
    <button class=${'nav-toggle' + (open ? ' open' : '')} aria-label="Toggle menu" onClick=${() => setOpen(!open)}><span /><span /><span /></button>
  </header>`;
}
...
function Footer() {
  const e = shared.emails;
  const mailto = (key) => html`<a href=${'mailto:' + e[key]}>${e[key]}</a>`;
  return html`<footer class="footer"><div class="footer-inner">
    <div class="footer-brand"><${SmartImg} src="./assets/Logo.png" alt="Cenoir" /><span>Cenoir Studios</span><${SocialLinks} /></div>
    <div class="footer-columns">
      ...
      <div class="footer-col"><h4>Contact</h4>${mailto('contact')}${mailto('careers')}${mailto('support')}</div>
      <div class="footer-col"><h4>Other</h4>${mailto('social')}${mailto('legal')}</div>
    </div>
    <p class="footer-copy">${shared.copyright}</p>
  </div></footer>`;
}
...
function HomePage() {
  const d = CONTENT.index;
  return html`<main class="page">
    <section class="hero"><div class="hero-content">
      <span class="hero-tag">${d.hero.tag}</span>
      <h1 dangerouslySetInnerHTML=${{ __html: d.hero.title }} />
      <p class="hero-desc">${d.hero.description}</p>
      <div class="hero-actions">
        <a href=${d.hero.cta_primary.href.replace('projects.html', '#/projects')} class="btn btn--primary">${d.hero.cta_primary.text}</a>
        <a href=${d.hero.cta_secondary.href.replace('about.html', '#/about')} class="btn btn--ghost">${d.hero.cta_secondary.text}</a>
      </div>
    </div></section>
    ...
    <section class="section"><div class="container">
      ...
      <div class="featured-grid">
        ${d.featured.images.map((img, i) => html`<${Reveal} key=${i} delay=${i + 1} className="featured-img-wrap">
    ...
    <section class="section" style=${{ background: 'linear-gradient(to bottom,transparent,rgba(212,137,42,0.02),transparent)' }}><div class="container">
      <div class="pillars-grid">
        ${d.pillars.map((p, i) => html`<${Reveal} key=${i} delay=${i} className="card">
          <span class="pillar-icon">${p.icon}</span><h3 class="pillar-title">${p.title}</h3><p>${p.text}</p><//>`) }
      </div>
    </div></section>
  </main>`;
}
...
function AboutPage() {
  ...
  return html`<main class="page">
    <section class="about-hero"><div class="container"><${Reveal}><div class="section-header">
      ...
      <p class="section-desc" style=${{ maxWidth: 620 }}>${d.hero.description}</p>
    </div><//></div></section>
    ...
    <section class="section" style=${{ paddingTop: 0 }}><div class="container">
      <div class="about-grid">${d.cards.map((c, i) => html`<${Reveal} key=${i} delay=${i} className="card">
        <h3>${c.title}</h3>${c.paragraphs.map((p, j) => html`<p key=${j}>${p}</p>`)}<//>`)}
      </div>
    ...
    <section class="section section--glow"><div class="container">
      <${Reveal}><div class="section-header">
        <span class="section-label">${d.journey.label}</span>
        <h2 class="section-title">${d.journey.title}</h2>
        <p class="section-desc">${d.journey.description}</p>
      </div><//>
      <${Reveal} className="timeline">${d.journey.milestones.map((m, i) => html`
        <div key=${i} class="timeline-item"><span class="timeline-year">${m.year}</span><h3>${m.title}</h3><p>${m.text}</p></div>`)}<//>

      <${Reveal} className="trailers-wrap">
        <div class="section-header" style=${{ marginTop: '4rem' }}>
          <span class="section-label">${d.trailers.label}</span>
          <h2 class="section-title">${d.trailers.title}</h2>
          <p class="section-desc">${d.trailers.description}</p>
        </div>
        <div class="video-grid">${d.trailers.videos.map(v => html`
          <a key=${v.id} href=${'https://youtu.be/' + v.id} target="_blank" class="video-thumb">
            ...
            <span class="video-label">${v.caption}</span></a>`)}</div>
      <//>
    </div></section>

    <section class="section"><div class="container">
      <${Reveal}><div class="section-header">
        <span class="section-label">${d.team.label}</span>
        <h2 class="section-title">${d.team.title}</h2>
        <p class="section-desc">${d.team.description}</p>
      </div><//>
      ${d.team.departments.map((dept, di) => html`<${Reveal} key=${di} delay=${Math.min(di, 3)} className="team-department">
        ...
        <div class="team-grid">${dept.members.map(m => html`<div key=${m.photo} class="team-member">
          ...
          <div class="team-fav">Favorite games: ${m.games}</div>
        </div>`)}</div>
      <//>`)}
    </div></section>
  </main>`;
...
function ProjectsPage() {
  const d = CONTENT.projects;
  const ns = d.nosains;
  const [slide, setSlide] = useState(0);
  const [modal, setModal] = useState(false);
  const [lightbox, setLightbox] = useState(null);
  const len = ns.gallery.length;
  ...
  const platforms = [
    { icon: '🪟', name: 'Windows', arch: 'x64 · .zip', url: ns.downloads.windows },
    { icon: '🍎', name: 'macOS', arch: 'Universal · .zip', url: ns.downloads.mac },
    { icon: '🐧', name: 'Linux', arch: 'x86_64 · .zip', url: ns.downloads.linux }
  ];
  ...
  return html`<main class="page">
      ...
      <${Reveal} className="project-showcase">
        <div class="project-top">
          <div class="project-info-panel">
            <h2>${ns.title}</h2>
            <span class="project-subtitle">${ns.subtitle}</span>
            <p>${ns.description}</p>
          ...
          <div class="carousel-panel">
            <div class="carousel-track" style=${{ transform: 'translateX(-' + (slide * 100) + '%)' }}>
              ${ns.gallery.map((img, i) => html`<div key=${i} class="carousel-slide"><${SmartImg} src=${img.src} alt=${img.alt} style=${{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>`)}
            </div>
            <div class="carousel-controls">
              <button class="carousel-btn" onClick=${() => setSlide((slide - 1 + len) % len)}>‹</button>
              <button class="carousel-btn" onClick=${() => setSlide((slide + 1) % len)}>›</button>
            </div>
            <div class="carousel-dots">${ns.gallery.map((_, i) => html`<button key=${i} class=${'carousel-dot' + (i === slide ? ' active' : '')} onClick=${() => setSlide(i)} />`)}</div>
          </div>
        </div>
        <div class="gallery-section">
          <p class="gallery-label">Concept Art Gallery</p>
          <div class="gallery-grid">${ns.gallery.map((img, i) => html`<div key=${i} class=${'gallery-thumb' + (i === slide ? ' active' : '')}
    ...
    ${lightbox && html`<div class="lightbox open" onClick=${() => setLightbox(null)}>
      ...
      <img src=${lightbox} alt="Concept art" /></div>`}
  </main>`;
}
...
function NewsPage() {
  ...
  return html`<main class="page">
    ...
    <section class="section" style=${{ paddingTop: 0 }}><div class="container">
      <div class="news-feed">${d.entries.map((e, i) => html`<${Reveal} key=${i} delay=${Math.min(i, 4)} className="news-item">
...
function RoleCard({ role }) {
  ...
  return html`<div class=${'role-card' + (open ? ' expanded' : '')}>
    <div class="role-header" onClick=${() => setOpen(!open)}>
      <div class="role-info"><h3>${role.title}</h3>
        <div class="role-meta"><span>${role.dept}</span><span>${role.location}</span><span>${role.type}</span></div>
      </div>
      <button class="role-toggle">+</button>
    </div>
    <div class="role-body"><div class="role-body-inner">
      ...
      <div class="role-apply-row"><a href=${'mailto:' + shared.emails.careers + '?subject=Application: ' + role.title} class="btn btn--primary btn--small">Apply Now →</a></div>
    </div></div>
  </div>`;
}

function CareersPage() {
  const d = CONTENT.careers;
  return html`<main class="page">
    ...
    <section class="section" style=${{ paddingTop: 0 }}><div class="container">
      <${Reveal}><div class="section-header">
        <span class="section-label">Open Roles</span><h2 class="section-title">Current opportunities</h2>
        <p class="section-desc">Click a role to see the full description and apply.</p>
      </div><//>
      <div class="open-roles">${d.roles.map((r, i) => html`<${Reveal} key=${i} delay=${Math.min(i, 4)}><${RoleCard} role=${r} /><//>`)}
      </div>
      <${Reveal} className="contact-block">
        <h3>Don't see your role?</h3>
        <p>We're always open to hearing from passionate people. Send us your portfolio and tell us what you'd bring to the team.</p>
        <a href=${'mailto:' + shared.emails.careers + '?subject=General Application'} class="btn btn--primary">Get in Touch</a>
      <//>
    </div></section>
  </main>`;
}
...
function useRoute() {
  ...
  useEffect(() => {
    const fn = () => { setRoute(window.location.hash.replace('#/', '') || ''); window.scrollTo(0, 0); };
...
function App() {
  ...
  return html`<${Navbar} current=${route} /><${Page} /><${Footer} />`;
}
